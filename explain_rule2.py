...
def explain_bio(treatment, effect, controls, species_name):
    """
    ...
    ctrl_phrase = f"under the influence of {', '.join(controls)}" if n_controls >= 2 else "independently"

    if effect > 0:
        if strength == "strong":
            interpretation = f"High {bio_name} strongly promotes {species_name} presence. This likely reflects a core habitat requirement {ctrl_phrase}."
        elif strength == "moderate":
            interpretation = f"Increasing {bio_name} moderately increases the occurrence probability. This may indicate partial environmental favorability {ctrl_phrase}."
        else:
            interpretation = f"{bio_name} has a weak positive effect, possibly secondary in importance."
    else:
        if strength == "strong":
            interpretation = f"{bio_name} strongly limits {species_name}'s distribution, potentially acting as a critical constraint {ctrl_phrase}."
        elif strength == "moderate":
            interpretation = f"Higher {bio_name} moderately suppresses presence, possibly reflecting ecological boundaries."
        else:
            interpretation = f"{bio_name} has a minor negative effect with little practical implication."
...
def generate_explanation_per_species(species_df, species_name):
    ...
    for _, row in species_df.iterrows():
        ...
        explanation = explain_bio(treatment, effect, controls, species_name)
        lines.append(explanation)

    # Optional: Add a summary of most impactful variables
    top = species_df.iloc[species_df['Effect'].abs().argsort()[::-1][:2]]
    top_vars = [BIO_DESC.get(t, t) for t in top['Treatment'].tolist()]
    top_vars_str = ", ".join(top_vars)
    summary = f"\n📈 Summary: {species_name}'s occurrence is primarily influenced by {top_vars_str}."
...
def generate_explanations_from_file(input_path, output_dir="outputs/species_explanations"):
    ...
    for species in df["Species"].unique():
        ...
        file_name = f"{species.replace(' ', '_')}_rule2_explanation.txt"
