...
# def notears_nonlinear_auto(X: np.ndarray,
#                             lambda1: float = 0.005,
#                             lambda2: float = 0.,
#                             model_type: str = 'mlp',
#                             h: int = 20,
...
#     d = X.shape[1]
#     if model_type == 'mlp':
#         model = NotearsMLP(dims=[d, h, 1], bias=True)
#         model = model.double() 
#     else:
...
#     return notears_nonlinear(model, X, lambda1, lambda2, max_iter, h_tol, rho_max, w_threshold)

# def notears_nonlinear_auto2(X: np.ndarray,
...
#                 W_est = notears_nonlinear(model, X, lambda1, lambda2, max_iter, h_tol, rho_max, w_threshold)
#                 num_edges = np.sum(np.abs(W_est) > w_threshold)
#                 max_weight = np.max(np.abs(W_est))

#                 print(f"   → Edges found: {num_edges}, Max weight: {max_weight:.4f}")
#                 tried_configs.append((lambda1, h, max_iter, num_edges))

#                 if 5 <= num_edges <= 50 and max_weight > 0.03:
#                     print("✅ Successful configuration found.")
#                     return W_est  

#     print("❌ All parameter combinations failed to produce non-zero graph.")
#     print("📋 Tried configurations:", tried_configs)
#     return np.zeros((d, d))  # fallback
...
def notears_nonlinear_auto(
    ...
    print("⚠️ All parameter combinations returned sparse/noisy graphs.")
