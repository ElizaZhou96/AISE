...
def run_notears_n_and_plot_colored(target_species):
    ...
    # W_est = notears_nonlinear_auto(X, lambda1=0.1, model_type='mlp', h=10)
    W_est = notears_nonlinear_auto(X)
    np.fill_diagonal(W_est, 0)
    print("✅ Max weight:", np.max(np.abs(W_est)))
    print("✅ Num nonzero edges (> 0.01):", np.sum(np.abs(W_est) > 0.01))
    ...
    used_nodes = list(G.nodes())
    node_colors = [bio_type_colors[get_bio_type(var)] for var in used_nodes]
    ...
    fig_path = f"outputs/{target_species.replace(' ', '_')}_NonL_DAG_colored.png"
    plt.savefig(fig_path, dpi=300, bbox_inches='tight')
    plt.close()

    return fig_path, top_edges, explanations, G

run_notears_n_and_plot_colored("Osmia parietina") #Osmia parietina & Ajuga reptans
